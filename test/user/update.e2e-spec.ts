...
describe("Anonymous can join as a new user", () => {
     ...
     */
    const JOIN_USER_ROUTE = `/${UserRouteName}${UserRoute.JoinUser}`;
    const GET_USER_ONE_BY_EMAIL = `/${UserRouteName}${UserRoute.GetUserOneByEmail}`;
    const UPDATE_USER = `/${UserRouteName}${UserRoute.UpdateUser}`;
    ...
    it("should join as a user", async () => {
        ...
        const response: UserIdDto = (
            await request(app.getHttpServer()).post(JOIN_USER_ROUTE).send(body)
        ).body;
    ...
    it("should update the user", async () => {
        const user = testUsers[0];
        const url = UPDATE_USER.replace(":id", user.id);

        const body: UpdateUserInputDto = {
            email: Random.email(),
            nickname: Random.nickname(),
        };
        ...
        expect(updatedUser?.nickname).toEqual(body.nickname);
    });
});
